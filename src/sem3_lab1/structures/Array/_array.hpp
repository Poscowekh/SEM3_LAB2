...
private:
    ...
    inline pointer _alloc(const int size) const noexcept{
        return pointer(malloc(elsize() * size));
    };
    ...
    void _realloc_back(const int new_alloc){
        ...
        if(new_alloc >= _size)
            memcpy(tmp, _data, elsize() * _size);
        else
            memcpy(tmp, _data, elsize() * new_alloc);
        free(_data);
        _data = tmp;
    ...
    void _dealloc_back(const int dealloc = 1){
        ...
        if(dealloc < _size){
            memcpy(tmp, _data + dealloc, elsize() * (_size - dealloc));
            _size -= dealloc;
        } else
            _size = 0;
        free(_data);
    ...
    void _realloc_front(const int new_alloc){
        pointer tmp = _alloc(new_alloc);
        memcpy(tmp + new_alloc - _get_buff_size() - _size, _data, elsize() * _size);
        free(_data);
        _data = tmp;
        _allocated = new_alloc;
    };
    void _dealloc_front(const int dealloc = 1){
        ...
        memcpy(tmp, _data + dealloc, elsize() * _size);
        _allocated -= dealloc;
        free(_data);
        _data = tmp;
    };
...
public:
    Array() : _size(0), _allocated(0), _data(0) {};
    Array(const int size) :
        _allocated(size + _get_buff_size()), _size(size) {
        if(size < 1)
            throw std::logic_error("\nArray exception: allocation of negative-sized memory");
        _data = pointer(malloc((_size + _get_buff_size()) * elsize()));
    };
    ...
    Array(const_pointer data, const int count) : Array(count) {
        memcpy(_data, data, elsize() * _size);
    };

    Array(const Array* other) : Array(other->_data, other->_size) {};
    Array(const Array& other) : Array(&other) {};
    Array(Array&& other) :
        _data(other._data), _size(other._size), _allocated(other._allocated) {
        other._data = 0;
    };
    ...
    inline int elsize() const noexcept {
        return sizeof(T);
    };
    void clear(){
    ...
    void copy_from_to(const_pointer data, const int count, const int from){
        ...
        memcpy(tmp, _data, elsize() * from);
        memcpy(tmp + from, data, elsize() * count);
        memcpy(tmp + from + count, _data + from, elsize() * (_size - from));
        free(_data);
        _data = tmp;
        _size += count;
    ...
    void set(const int index, const_pointer data, const int from_index = 0){
        memcpy(_data + index, data + from_index, elsize());
    };
    ...
    void push_back(const_pointer data){
        _realloc_check();
        memcpy(_data + _size, data, elsize());
        ++_size;
    };
    ...
    void push_front(const_pointer data, const int count = 1){
        ...
        memcpy(_data, data, elsize() * count);
        _size += count;
    };
    ...
    void remove(const int index){
        if(index < 0 || index >= _size)
            ...
            memcpy(tmp, _data, elsize() * index);
            memcpy(tmp + index, _data + index + 1, elsize() * (_size - index));
            free(_data);
            _data = tmp;
            _size--;
        };
    ...
    void insert(const_pointer data, const int index){
        if(index < 0 || index > _size)
            ...
            pointer tmp = _alloc(_size + 1 + _get_buff_size());
            memcpy(tmp, _data, index * elsize());
            memcpy(tmp + index, data, elsize());
            memcpy(tmp + index + 1, _data + index, elsize() * (_size - index));
            free(_data);
            _data = tmp;
            _size++;
        };
