...
namespace Testing {
    ...
    struct __sample {
        ...
        __sample() : data(0), size(0) {}
        __sample(const int _size) : __sample(_size, time(0)) {}
        __sample(const int _size, const int seed)
            : size(_size),
        ...
        __sample(__sample&& other) : size(other.size), data(other.data) {
            other.data = 0;
        };
