...
namespace Testing {
    ...
    _sample sorted_sample(const int& size) {
        _sample result = _sample(size);
        int r = -size;
    ...
    template<typename Container, typename dummy1, typename dummy2>
    //for traversable as trees
    bool _assert_equal_traverse(const Container& container1, const Container& container2, const dummy1&, const dummy2&){
        ...
        container1.traverse([&flag, &container2](const dummy1& index, const dummy2&) {
            if(!container2.find_index(index))
                flag = false;
        });
